...
compote component pack --name {{COMPONENT_NAME}} --version X.Y.Z
compote component upload --namespace {{NAMESPACE}} --name {{COMPONENT_NAME}}
```
...
```bash
idf.py add-dependency "{{NAMESPACE}}/{{COMPONENT_NAME}}^X.Y.Z"
```
...
```yaml
dependencies:
  {{NAMESPACE}}/{{COMPONENT_NAME}}: "^X.Y.Z"
```
