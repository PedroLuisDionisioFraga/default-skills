...
- Keep lock hold time short; do not log heavily while holding a mutex, and never hold one across a driver call — that is where priority inversion and callback deadlocks come from.
- Prefer one owner task for a contended peripheral over many tasks sharing a mutex. A mutex protects *data* from being torn; it does nothing for a `read -> decide -> write` *sequence* that two tasks interleave, which is the failure that actually corrupts outputs. `references/single-owner-io.md` covers the full pattern: naming the owner, the arbitrate-vs-execute boundary, the four ownership zones, and the init ordering.
...
- Confirm each contended peripheral has one owning task; flag `read -> decide -> write` sequences spanning a blocking call, and read-modify-writes performed outside the owner.
