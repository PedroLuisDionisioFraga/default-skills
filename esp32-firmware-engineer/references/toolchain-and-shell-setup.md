...
- Before running `idf.py build` (or a project wrapper), verify ESP-IDF is actually usable:
  - `idf.py` resolves
...
If any fail:
- source the ESP-IDF environment (`export.sh` / `~/.esp_idf_env` on macOS/Linux, `export.ps1` or the ESP-IDF PowerShell shortcut on Windows)
