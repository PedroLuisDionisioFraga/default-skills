...
- Name the single owning component for each hardware output, and keep its worker task the only writer; every other context requests. Components below the owner may keep internal timing to execute a decided command, but must not arbitrate between callers (`references/single-owner-io.md`).
...
- Check how many contexts can write each peripheral; an output with no single owning task is a finding in itself.
- Check `read -> decide -> write` sequences for blocking calls in between (logging, NVS, network, lock acquisition) that make the sampled state stale, for read-modify-writes such as `set(!get())` outside the owner, and for locks held across driver calls.
...
- Use `idf.py build` from the project root, unless the project ships its own wrapper. `fw_sbc-wxxx` does not — see `QUICK_CHECK.md` section 5.
